...
# 4.1 / Table 4: projection dimension.
for D in 32 64 128 256 512; do
  $T $BASE --freeze-layers 2 --proj-dim "$D" \
     --output-dir "Result/04_Ablation/01_Projection_Dim/d${D}"
done

# 4.2 / Table 5: FOROH score loss.
for LOSS in huber l1 smooth_l1 mse; do
  $T $BASE --freeze-layers 2 --loss-fn "$LOSS" \
     --output-dir "Result/04_Ablation/02_Loss/${LOSS}"
done

# Learnable severity axis vs fixed random axis.
$T $BASE --freeze-layers 2 \
   --output-dir Result/04_Ablation/03_Axis/learnable
$T $BASE --freeze-layers 2 --fixed-w \
   --output-dir Result/04_Ablation/03_Axis/fixed_random

# Projector necessity.
$T $BASE --freeze-layers 2 \
   --output-dir Result/04_Ablation/04_Projector/with
$T $BASE --freeze-layers 2 --no-projector \
   --output-dir Result/04_Ablation/04_Projector/without

# Freeze-layer ablation from the draft table.
